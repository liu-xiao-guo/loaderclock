...
# 1) run make once to create and update the po/loaderclock.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/loaderclock.pot po/en.po
# 3) edit the po file
...
APP_ID=loaderclock.liu-xiao-guo
...
po/loaderclock.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/loaderclock.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
